...
def main():
    ...
    if not sys.stdin.isatty():
        parser.error("run in a terminal to enter the remote account password")
    ...
    with tempfile.TemporaryDirectory(prefix="tmax-integration-") as directory:
        ...
        config.write_text(json.dumps({args.label: {"destination": args.host,
        ...
        try:
            ...
            remote_module.setup()
            if not remote_module.auth.unlock(remote_module.RUNTIME, args.label, remote_module.hosts()[args.label],
                                             remote_module.SELF + ["auth-guard"]):
                raise RuntimeError("password unlock failed; configure key + password on the test host first")
            lease = remote_module.auth.connection(remote_module.RUNTIME, args.label, remote_module.hosts()[args.label])
            ssh = ["ssh", "-S", lease["socket"], "-o", "ControlMaster=no", "-o", "ProxyCommand=false", args.host]
            ...
            time.sleep(0.5)
            send(b"\x01s")
            wait_for(lambda: "tree-mode" in tmux("display-message", "-p", "-t", active_session(), "#{pane_mode}"), "native local session tree opens")
            assert tmux("list-sessions", "-F", "#{session_name}") == "local-test"
            send(b"q")
            wait_for(lambda: not tmux("display-message", "-p", "-t", active_session(), "#{pane_mode}"), "native tree closes")
            send(b"\x01 ")
            wait_for(lambda: b"remote-test" in terminal_output, "popup lists remote session")
            send(b"iremote-test")
            time.sleep(1)
            send(b"\r")
            wait_for(lambda: proxy in tmux("list-clients", "-F", "#{session_name}"), "popup selects remote session")
            ...
            wait_for(lambda: proxy not in tmux("list-sessions", "-F", "#{session_name}"), "ended remote session removes its local proxy")
            print("PASS: preserves prefix, unrelated hooks, and reload behavior", flush=True)
        except Exception:
            print("TERMINAL:", bytes(terminal_output[-4000:]))
